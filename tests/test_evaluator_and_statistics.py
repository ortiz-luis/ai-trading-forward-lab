...
from engine.statistics import build_statistics
...
def test_stop_loss_closes_early():
    result = evaluate_long_decision(
...
def test_statistics_track_money_and_drawdown_separately_from_score():
    d = buy_decision()
    ...
    stats = build_statistics([win, loss], starting_capital_eur=1000, no_trades=2, errors=1)
    assert stats.wins == 1
